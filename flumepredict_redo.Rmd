...
C <- 25 #g/cm^3
C <- C * 1000 #convert to kg per cubic meter
...
setCond <- function(data){
  data$C <- 25 * (1000) #kg/m^3 SSC
  data$area <- 1*1 #1x1 m test area
  data$n_c <- data$c_density/data$area # number of collecters/m^2
  data$rho_w <- data$f_density * (1000) #density of fluid (kg/m^3)
  data$rho_p <- data$p_density * (1000) #density of particle (kg/m^3)
...
muddData$d_p_um <- mudd_data$d_p_cm * 10000

muddData$velocity_m <- mudd_data$ave_velocity / 100
...
a40 <- ggplot(data_40, aes(x = ave_velocity, y = frontal_area_unit_volume, z = tideC)) + 
  geom_contour(aes(color = ..level..), bins = 15) +
  geom_label_contour(skip = 0, rotate = FALSE) +
  theme_pubclean() + 
  theme(axis.title.x = element_blank()) +
  labs(y = "Capture") +
  scale_color_viridis_c(name = "Sedimentation (cm/tide)",limits = c(0,0.006), direction = 1, option = "C", breaks = c(0,0.006,0.01)) 

b40 <- ggplot(data_40, aes(x = ave_velocity, y = frontal_area_unit_volume, z = tideS)) + 
  geom_contour(aes(color = ..level..), bins = 8) +
  geom_label_contour(skip = 0, rotate = FALSE) +
  theme_pubclean() + 
  theme(axis.title.x = element_blank()) +
  labs(y = "Settling") +
  scale_color_viridis_c(limits = c(0,0.006), direction = 1, option = "C", aesthetics = "colour", breaks = c(0,0.006,0.01)) 

c40 <- ggplot(data_40, aes(x = ave_velocity, y = frontal_area_unit_volume, z = tideTotal)) + 
  geom_contour(aes(color = ..level..), bins = 15) +
  geom_label_contour(skip = 0, rotate = FALSE) +
  theme_pubclean() + 
  theme(axis.title.x = element_blank()) +
...
annfig40 <- annotate_figure(fig40, top = text_grob(sub40a), left = text_grob(ytext, rot=90), bottom = text_grob(xtext))
...
a60 <- ggplot(data_60, aes(x = ave_velocity, y = frontal_area_unit_volume, z = tideC)) + 
  ...
  theme(axis.title.x = element_blank()) +
  labs(y = "Capture") +
  scale_color_viridis_c(name = "Sedimentation (cm/tide)", limits = c(0, 0.0125), direction = 1, option = "C", breaks = c(0,0.0125))  
...
ggplot(debug_dataa, aes(x = ave_velocity, y = frontal_area_unit_volume, z = dzdt)) + 
  ...
  theme(axis.title.x = element_blank(), 
        legend.key = element_blank()) +
  labs(y = "Frontal Area/Unit Volume", x = "Velocity") +
  scale_color_viridis_c(name = "Sedimentation (m/sec)", limits = c(0.01, 0.02), direction = 1, option = "C", breaks = c(0.01, 0.02))
