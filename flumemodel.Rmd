...
png("graphics/noc5.png")
ggplot(logdata, aes(x = logY, y = prednoc5)) + 
  geom_smooth(method = "lm", color = "black") +
  geom_point(shape = 19, size = 5, alpha = .8, aes(color = Re_c)) +
  theme_minimal() + 
  labs(title = "All Runs", y = "predicted eta", x = "observed eta") +
...
Palmer <- function(Re, #Reynolds Number Re_c
  ...
  return(eta) #adjust to return either eta or ln(eta)
}
...
(mean((logdata$predeta-logdata$eta)**2))**(1/2) #for comparison: 0.002 mean residual from the synthesis model

palmer_rmse <- (mean((logdata$palmer-logdata$eta)**2))**(1/2) #10.17 mean residual
```
...
fauria_r2 <- cor(logdata$logY, logdata$fauria)**2  #0.31 ; same ; same

#Residuals

fauria_rmse <- (mean((logdata$fauria-logdata$eta)**2))**(1/2) #7.61 mean residual
```
...
gather_a <- logdata %>%
  dplyr::select(c(Re_c, eta, predeta)) %>% #just the pertinent variables
  mutate(author = "This paper") %>% #set author name to label on plot later
  mutate(predeta = as.numeric(predeta)) #change from named numeric to numeric

gather_b <- logdata %>%
  dplyr::select(c(Re_c, eta, palmer)) %>%
  rename(predeta = palmer) %>% #change variable name to match for later rbind
  mutate(author = "Palmer") %>%
  mutate(predeta = as.numeric(predeta))

gather_c <- logdata %>%
  dplyr::select(c(Re_c, eta, fauria)) %>%
  rename(predeta = fauria) %>%
  mutate(author = "Fauria") %>%
  mutate(predeta = as.numeric(predeta))
...
comp1plot <- ggplot(gather_data) + geom_point(aes(y = predeta, x = eta, color = Re_c, shape = author), size = 6, alpha = 0.6) +
  labs(x = xlab, y = ylab) +
 # lims(x = c(-15, 0)) +
  lims(y = c(0, 0.05)) +
  theme_pubclean() +
...
ggsave(filename = "graphics/all_eqn_nonlog_b.png", plot = comp1plot)
