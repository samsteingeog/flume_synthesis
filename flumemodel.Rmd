...
lm_eqn <- function(data){
    ...
    eq <- substitute(~~italic(r)^2~"="~r2, #italic(logY == int + c1*logX1 + c2*logX2 + c3*logX3 + c4*logX4 + c5*logX5) *","
...
sing <- ggplot(single, aes(x = logY, y = predlogeta)) + geom_smooth(method = "lm", color = "black") +
  geom_point(shape = 19, size = 5, alpha = .8, aes(color = Re_c)) +
  theme_minimal() + 
  labs(title = "g) Single Collector Studies", y = "predicted log(eta)", x = "observed log(eta)") +
  geom_text(x = -8, y = -2, label = lm_eqn(biofilm), parse = TRUE, size = 5) +
  scale_color_viridis_c(option = "plasma")
...
png("graphics/Re1.png")
ggarrange(Re1_plot)
dev.off()

png("graphics/Re2.png")
ggarrange(Re2_plot)
dev.off()
...
pred_all <- pred_r_squared(total_lm)
pred_bio <- pred_r_squared(biofilm_lm)
pred_re1 <- pred_r_squared(Re1_lm)
